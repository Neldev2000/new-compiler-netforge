    ...
    if (get_block()) {
        ...
        for (const Statement* stmt : block->get_statements()) {
            ...
            if (const SectionStatement* section = dynamic_cast<const SectionStatement*>(stmt)) {
                ...
                // Get the raw interface name from section
                std::string interface_name = section->get_name();
                
                fprintf(stderr, "DEBUG: Processing interface subsection '%s'\n", interface_name.c_str());
                
                // Remove trailing colon if present
                if (!interface_name.empty() && interface_name.back() == ':') {
                    interface_name = interface_name.substr(0, interface_name.size() - 1);
                    fprintf(stderr, "DEBUG: After removing colon, interface name = '%s'\n", interface_name.c_str());
                }
                ...
                if (colon_pos != std::string::npos) {
                    ...
                    fprintf(stderr, "DEBUG: After further colon processing, interface name = '%s'\n", interface_name.c_str());
                }
                ...
                // If the interface name is now valid, continue processing
                fprintf(stderr, "DEBUG: Processing interface '%s'\n", interface_name.c_str());
        ...
        if (!handled_subsections) {
            fprintf(stderr, "No subsections found, using fallback logic.\n");
            
            // Interface properties we need to extract
            std::string name = "";
            std::string type = "";
            std::string mtu = "";
            std::string disabled = "";
            ...
            // If name is not set, use "ether1" as default
            if (name.empty()) {
                name = "ether1";
            }
            ...
            // Generate MikroTik commands based on interface type and properties
            if (type == "ethernet") {
                ...
                if (other_props.count("ports")) {
                    ...
                    while ((pos = ports.find(',')) != std::string::npos) {
                        ...
                        if (!port.empty()) {
                            ...
                            result += "/interface bridge port add bridge=" + name + " interface=" + port + "\n";
                        }
                        ports.erase(0, pos + 1);
                    }
                    ...
                    if (!ports.empty()) {
                        ...
                        result += "/interface bridge port add bridge=" + name + " interface=" + ports + "\n";
                    }
                }
            } else if (type == "loopback") {
                // Create loopback interface
                result += "/interface add name=" + name + " type=loopback";
                if (!disabled.empty()) result += " disabled=" + disabled;
                if (!comment.empty()) result += " comment=\"" + comment + "\"";
                result += "\n";
            } else if (type == "bonding") {
            ...
            // Add interface lists if specified
            if (other_props.count("lists") && !name.empty()) {
                std::string lists = other_props["lists"];
                // Similar processing as for bridge ports
                if (lists.size() >= 2 && lists.front() == '[' && lists.back() == ']') {
                    lists = lists.substr(1, lists.size() - 2);
                ...
                while ((pos = lists.find(',')) != std::string::npos) {
                    ...
                    if (!list.empty()) {
                        ...
                        result += "/interface list member add list=" + list + " interface=" + name + "\n";
                    }
                    lists.erase(0, pos + 1);
                }
                ...
                if (!lists.empty()) {
                    ...
                    result += "/interface list member add list=" + lists + " interface=" + name + "\n";
                }
            }
        }
    }
