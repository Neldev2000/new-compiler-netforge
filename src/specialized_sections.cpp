...
std::tuple<bool, std::string> IPSection::validate() const noexcept {
    ...
    for (const Statement* stmt : block->get_statements()) {
        ...
        if (subsection) {
            ...
            // Validate route subsection
                ...
                for (const Statement* route_stmt : subsection->get_block()->get_statements()) {
                    ...
                    if (route_section) {
                        ...
                        for (const Statement* route_detail : route_section->get_block()->get_statements()) {
                            ...
                            if (detail_prop) {
                                ...
                                // Add validation for other route properties if needed
                            }
                        }
            ...
            // Add validation for other subsections (firewall, DNS, etc.) if needed
        }
        // Check for direct properties under IP section
        else if (const PropertyStatement* prop = dynamic_cast<const PropertyStatement*>(stmt)) {
            const std::string& prop_name = prop->get_name();
